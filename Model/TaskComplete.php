...
class TaskComplete extends AppModel
{
    ...
    public function getAllRequestsPending($sp = 0, $limit = 20)
    {
        return $this->Query("SELECT * FROM $this->db2 WHERE status = '0' ORDER BY id DESC LIMIT $sp,$limit ;")->fetch_all(1);
    }
    ...
    public function getDetailsByIdRequest($id)
    {
        return $this->Query("SELECT * FROM $this->db2 WHERE id = '$id';")->fetch_array(1);
    }
    ...
    public function getFieldRequest($id, $field)
    {
        return $this->Query("SELECT $field FROM $this->db2 WHERE id = '$id' ;")->fetch_array(1);
    }
    ...
    public function saveFieldRequest($field,$value)
    {
        if($this->id != "0"){
            $result = $this->Query("UPDATE $this->db2 SET $field = '$value' WHERE id = '$this->id'");
        }else{
            $result = false;
        }
        return $result;
