...
function openeditappimages(id) {
    //alert('s');
    window.location = 'dashboard.php?p=editappimages&id=' + id;

}

function add_app_images(id) {
    var popup = document.getElementById("PopupParent");
    ...
    xmlhttp.open("GET", "ajex-events.php?q=addappimages&id=" + id);
    xmlhttp.send();
}
...
jQuery(document).on("click", ".inboxRow", function() {
    ...
    $.ajax({
        url: "ajex-events.php?q=userChat&rid=" + rid + "&user_id=" + user_id,
...
jQuery(document).on("click", "#deleteComment", function() {
    ...
    $.ajax({
        url: "process.php?action=deleteComment&comment_id=" + comment_id,
        success: function(result) {
